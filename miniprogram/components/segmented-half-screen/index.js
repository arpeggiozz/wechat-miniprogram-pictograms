...
Component({
  ...
  lifetimes: {
    ...
    attached() {
      const nextStatusBarHeight = statusBarHeight + this.data.bottomOffset;
      this.setData({
        statusBarHeight: nextStatusBarHeight,
        height: screenHeight - nextStatusBarHeight,
      })
    },
    ready() {
  ...
  methods: {
    ...
    shouldScrollViewResponse(pointerEvent) {
      ...
      if (this.transY.value > this.data.statusBarHeight) return false
    ...
    handlePan(gestureEvent) {
      ...
      if (gestureEvent.state === GestureState.ACTIVE) {
        ...
        // 只能在 [this.data.statusBarHeight, screenHeight] 之间移动
        const destination = clamp(curPosition + gestureEvent.deltaY, this.data.statusBarHeight, screenHeight)
      ...
      if (gestureEvent.state === GestureState.END || gestureEvent.state === GestureState.CANCELLED) {
        if (this.transY.value <= screenHeight / 2) {
          ...
          if (this.upward.value) {
            this.scrollTo(this.data.statusBarHeight)
          } else {
    ...
    handleTouchEnd() {
      ...
      if (this.transY.value === this.data.statusBarHeight) {
        this.lastTransY = this.data.statusBarHeight
        this.scrollTo(screenHeight / 2)
